...
2. cn_complex.py generates the maximal simplices of the strong Morse complex on the cycle C_n with n edges.
3. xm_complex.py generates the maximal simplices of the complex X(m) defined in the paper. We have X(2m) = SM(P_m).
4. ym_complex.py generates the maximal simplices of the complex Y(m) defined in the paper.
5. en_complex3.py generates the maximal simplices of the subcomplex E(n) in SM(C_n), where C_n is the cycle with n edges, consisting of simplices having only even numbered vertices.
